				 ...
				 )
{
	...
	// data index = 724 +10(data)
	int fOffset_firstCarrier = vhw_calcFirstCarrier(fftSize);
	
	for(int i=0 ; i<fftSize; i++)
	{	
		short * ptrbit = (short*)(&vtx_freqBuffer[Ns][0][i]);

		if(i >= fOffset_firstCarrier && i <fOffset_firstCarrier+5)
		{
			if(*ptrbit == 1)
			{
				*ptrbit = 5792;
				...
				(onOffCfg = vtx_readOnOff()) == 1) )
		{
			...
			if (rtCfgInfo.status == txst_idle)
			{
				...
				if (sendFlag == 1)
				{
					...
					{	
						//printf("%d ", vtx_rfRegPtr->sharedReg.txData[rtCfgInfo.slot_nr_sch][0][i]*2);
						printf("%d ", vtx_buffer[rtCfgInfo.slot_nr_sch][i+88]);
						//printf("%d ", ((short*)(&vtx_buffer[rtCfgInfo.slot_nr_sch][i]))[1]);	
					}
					printf("\n");

					
               				//get dumpData when tx
                  			...
                  			vhw_dumpData(&vtx_rfRegPtr->sharedReg.txData[rtCfgInfo.slot_nr_sch][0][0], slotSize, "TXDATA.txt");
