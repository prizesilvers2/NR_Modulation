...
→ Preamble detection에 성공했더라도 Peak값이 임계값보다 낮을 경우 실패로 값을 반환하게 되는데 이때, VTX_ONLINE_THRES_PREAMBLEDETECTION, VTX_DEFAULT_THRES_PREAMBLEDETECTION은 임계값을 결정하는 변수로 사용됨.
